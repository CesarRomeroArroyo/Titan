<?php
	...
	$sql="SELECT * FROM shopping_suppliers_bills WHERE idunico='".$_GET["id"]."'";   
	$cabecera = consulta::convertir_a_array(consulta::ejecutar_consulta($sql));

	$sql="SELECT * FROM shopping_suppliers_bills_details WHERE idunico='".$_GET["id"]."'";   
...
<page_header>
		<table border='0' style="font-size: 7pt;width: 100px">
			<tr>
				...
				<td style="text-align: center; font-size: 13px;   width: 150px; height: 120px">FACTURA DE COMPRA<BR/>No <b><?php echo $cabecera[0]["id"] ?></b></td>
...
<div style="width: 900px">
	...
	<table style="font-size: 9pt; width:900px;"  >
		<tr>          
			...
			<td style='width: 40px;'><b>DTO1</b></td>
			<td style='width: 40px;'><b>DTO2</b></td>
	...
	?>
		<tr>
			...
			<td><?php echo number_format(($detalles[$i]['precio_uni']),0) ?></td>
			<td><?php echo number_format($detalles[$i]['desc1'],0) ?></td>
			<td><?php echo number_format($detalles[$i]['desc2'],0) ?></td>
			<td><?php echo $detalles[$i]['iva'] ?></td>
			<td><?php echo number_format($detalles[$i]['preciototal'],0) ?></td>
		</tr>
	<?php    
		}
	?>
